        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button 
              ...
              onTouchStart={() => {}} 
              className={`h-14 sm:h-16 flex flex-col items-center justify-center gap-0.5 sm:gap-1.5 px-1 sm:px-6 rounded-full transition-all group flex-1 sm:flex-none cursor-pointer ${isActive ? 'bg-[#F7F7F5]' : 'hover:bg-[#F7F7F5] active:bg-[#F0F0F0]'}`}
            >
              ...
              <span className={`text-[10px] sm:text-[13px] font-bold tracking-widest transition-colors duration-300 truncate ${isActive ? 'text-gray-900' : 'text-gray-400 group-hover:text-gray-900 group-active:text-gray-900'}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
